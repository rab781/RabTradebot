...
## 2026-07-19 - [Optimize API DataFrame Conversions]
**Learning:** In `TradingViewService`, mapping provider API responses (Binance, CryptoCompare, AlphaVantage) to column-based DataFrames using multiple chained `.map()` calls on intermediate object arrays causes extreme O(N) allocation and garbage collection overhead. Since this is on the hot path for data fetching and real-time polling, the redundant loops significantly increase memory pressure.
**Action:** When extracting separate column arrays (like open, high, low, close) from a list of external provider objects to build a DataFrame, never use repeated `.map()` calls. Instead, pre-allocate the column arrays (`new Array(len)`) and populate them within a single `for` loop to reduce iteration complexity to O(N).
