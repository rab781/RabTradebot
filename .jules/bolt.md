...
## 2026-05-28 - [O(N) Optimization for Variance/Standard Deviation]
**Learning:** In `StrategyOptimizer.analyzeParameterSensitivity`, calculating the standard deviation of an array via chained `.map()`, `Math.max(...array)`, and multiple `.reduce()` calls causes heavy memory allocation overhead, O(N) iteration passes per function call, and introduces severe `RangeError: Maximum call stack size exceeded` vulnerabilities when spreading large arrays.
**Action:** When calculating variance and standard deviation, consolidate all metrics (`sum`, `sumSq`, `max`) into a single O(N) `for` loop, eliminating array cloning and spread operations. Derive variance mathematically in O(1) step time: `(sumSq / len) - (mean * mean)`. Always wrap variance in `Math.max(0, variance)` before `Math.sqrt` to prevent floating point edge-case NaNs.
