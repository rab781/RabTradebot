...
## 2024-05-25 - [Optimize Array padding and concatenation]
**Learning:** In technical analysis calculations, repeated intermediate array creation via `new Array(pad).fill(NaN).concat(arr)` triggers considerable garbage collection overhead when generating indicators on thousands of candles, due to O(N) intermediate structures for every feature (e.g. `concat` copies both arrays).
**Action:** Replace `new Array(pad).fill(NaN).concat(arr)` with an in-place array pre-allocation and assignment, optionally combined with mapped indicators to eliminate array copying entirely.
