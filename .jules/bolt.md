...
## 2026-05-18 - [Optimize Parameter Importance and Sensitivity Iterations]
**Learning:** In `StrategyOptimizer.analyzeResults` and `StrategyOptimizer.parameterSensitivity`, calculating aggregate metrics (like mean, variance, standard deviation) using multiple consecutive `.map()` and `.reduce()` operations over the exact same dataset causes massive O(M*N) object allocation overhead during parameter grid search evaluations. In addition, using `Math.max(...performances)` with the spread operator on these optimization grid neighborhoods can trigger stack overflows if the neighborhood slice grows too large.
**Action:** Replace sequential `.map()` and `.reduce()` chains with single-pass, pre-allocated `for` loops (`new Array(len)`) that aggregate values (`sumScore`, `sumScoreSq`) while extracting the column values. Compute standard deviation mathematically using the variance formula: `(sumSq - (sum * sum / len)) / len` directly on the scalars without needing a second iteration pass. Avoid using the spread operator on dataset performance arrays; instead, compute the running max inside the same O(N) loop.
