...
## 2026-05-31 - [Optimization in Parameter Sensitivity Analysis]
**Learning:** Found multiple array passes (.map, .reduce) and expensive Math.max spread syntax being used in the parameter sensitivity calculation of the StrategyOptimizer. When combined together on potentially large datasets (like neighborhood evaluations), these can add noticeable overhead and maximum call stack errors due to closure allocations and spread limitations.
**Action:** Replaced multiple O(N) chained array methods and spread usage with a single, pre-allocated O(N) `for` loop to compute the `bestPerf`, `mean`, and `stdDev` simultaneously. We applied the identity `Var(X) = E[X^2] - (E[X])^2` for one-pass variance calculation.
