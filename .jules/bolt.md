...
## 2026-06-05 - [Optimize Parameter Importance Extractions]
**Learning:** In `StrategyOptimizer.analyzeResults`, repeated nested iterations inside an `Object.keys()` loop such as `results.map(r => r.params[paramName])` and `results.map(r => r.score)` combined with subsequent `results.reduce` calls cause immense overhead (O(P * 2N + 2N) where P is parameters).
**Action:** When computing statistical metrics across multiple parameters, always pre-calculate shared arrays (like mapping overall scores and deriving variance mathematically `Math.max(0, (sumScoreSq / len) - (avgScore * avgScore))`) in a single pass outside the parameter loop, and use pre-allocated arrays `new Array(len)` inside the loop.
