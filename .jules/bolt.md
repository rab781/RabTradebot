...
## 2026-06-03 - [Consolidating Statistical Math into Single Loops]
**Learning:** Using chained `.reduce()` passes inside heavily evaluated statistical calculation loops (e.g., in `strategyOptimizer.ts` for computing `avgScore` and `scoreStd`) over thousands of iterations causes unnecessary array traversals, function closure allocations, and garbage collection pressure.
**Action:** When calculating statistics like mean and standard deviation over numerical arrays, combine sum and sum of squares into a single `for` loop traversal. Derive variance outside the loop using mathematically sound equivalents to prevent repetitive array iteration.
