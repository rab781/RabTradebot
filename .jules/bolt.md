...
## 2026-06-04 - [Replace array mapping with single-pass loops in StrategyOptimizer parameter importance]
**Learning:** In `StrategyOptimizer.parameterSensitivity` (or similar correlation/importance calculation methods), mapping arrays of parameters and scores to create new intermediate arrays repeatedly inside a loop generates significant O(N) allocation overhead per parameter iteration. Combined with chained `reduce` calls to compute standard deviations, this creates a major performance drag during parameter optimization routines.
**Action:** Replace `results.map()` and chained `results.reduce()` calls in correlation mapping loops with pre-allocated arrays and single-pass accumulation loops. This allows O(1) allocation and minimizes closure execution overhead in the core parameter evaluation loop.
