...
## 2026-04-21 - [Prevent Call Stack Size Exceeded Exceptions in Database Statistics]
**Learning:** Using `Math.max(...array)` and `Math.min(...array)` with the spread syntax in JavaScript throws "Maximum call stack size exceeded" exceptions if the array contains too many elements (usually > 100k, but can be much lower depending on the engine). In database functions like `getUserTradeStats`, trade histories can easily exceed this limit.
**Action:** Replace `Math.max(...array)` and `Math.min(...array)` alongside chained array operations (`.map`, `.filter`, `.reduce`) with a single pre-allocated iterative `for` loop to accumulate metrics safely and efficiently without triggering stack overflows or massive garbage collection.
