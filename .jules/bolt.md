...
**Learning:** In `src/services/databaseService.ts`, statistical methods like `getUserTradeStats` used chained functional methods (`.map()`, `.filter()`, `.reduce()`) to process arrays of database records. Not only did this create O(N * 3) time complexity and excess garbage collection from intermediate arrays, but using the spread operator (`Math.max(...profits)`) on large datasets threw a `Maximum call stack size exceeded` RangeError.
**Action:** Always replace chained array operations and `Math.max/min(...array)` with a single, unified O(N) `for` loop when calculating aggregate statistics (like counts, sums, min, max) on datasets that can grow unbounded.
