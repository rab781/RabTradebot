...
## 2024-07-19 - Fast column extraction
**Learning:** Extracting separate column arrays (open, high, low, close) from a list of candlestick objects using repeated `.map()` calls incurs significant O(C * N) traversal overhead.
**Action:** When building a DataFrame, pre-allocate the column arrays (e.g. using `new Array(len)`) and populate them within a single `for` loop to reduce iteration complexity to O(N).
