...
## 2026-05-18 - [Optimize Multiple Array Passes in Statistical Calculations]
**Learning:** In statistical methods like `calculateCorrelation` (`StrategyOptimizer`), executing multiple `.reduce()` chains over the same large arrays (e.g., to calculate `sumX`, `sumY`, `sumXY`, etc.) incurs massive allocation and closure execution overhead (O(N) * 5 passes).
**Action:** When computing multiple related metrics from identical arrays, replace chained higher-order array methods with a single pre-allocated `for` loop that accumulates all values simultaneously in one pass.
