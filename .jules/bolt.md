...
## 2026-06-13 - [Optimize Array Operations in Machine Learning and Strategies loops]
**Learning:** In \`src/ml/simpleGRUModel.ts\` and \`src/services/strategyOptimizer.ts\`, calculating aggregated metrics (like mean, variance, and min/max limits) using chained array operators such as \`map()\`, multiple \`reduce()\`, or the spread operator \`Math.max(...array)\` leads to performance degradation because it traverses the array multiple times, allocates intermediate arrays and may throw \`RangeError\` for very large collections.
**Action:** Replace multiple chained array manipulations with a single \`for\` loop which processes the elements sequentially in O(N) time and calculates limits and cumulative totals incrementally (e.g. \`sum\`, \`sumSquare\`, \`max\`, \`min\`). Use mathematical equivalence \`Var(X) = E[X^2] - (E[X])^2\` (clamping to zero to avoid floating point \`NaN\` during \`Math.sqrt()\`) to resolve variance.
