...
## 2026-07-08 - [O(N) Optimization for Pearson Correlation Calculation and array Percentile calculations]
**Learning:** In `StrategyOptimizer.calculateCorrelation`, calculating the sum of arrays via chained `.reduce()` calls on identical data structures forces the JS engine to traverse the array multiple times, causing O(5N) iteration overhead and allocating multiple closures per function call. Also, using array methods `map()` multiple times per array loop. And in calculating Monte Carlo test percentiles using nested sorts repeatedly.
**Action:** Consolidate multiple reduce loops into a single O(N) loop with mathematical standard derivation metrics, hoist array extraction, and sort percentiles precisely once.
