...
## 2026-06-03 - [Optimize Percentile Calculations]
**Learning:** When calculating multiple percentiles (p5, p25, median, etc.) from arrays like profit or drawdown distributions, performing `.sort()` for each percentile calculation results in redundant O(N log N) overhead, which adds up in large simulation loops (like Monte Carlo testing).
**Action:** Sort the distribution array exactly once and extract all required percentiles simultaneously using pre-calculated index lookups (`sorted[Math.ceil((p / 100) * len) - 1]`) in O(1) time.
