...
## 2026-04-16 - [Database Service Metric Calculation Optimization]
**Learning:** In `src/services/databaseService.ts`, calculating trade statistics and prediction accuracy metrics used chained array methods (`.map()`, `.filter()`, `.reduce()`) and `Math.max(...)/Math.min(...)` which spreads array contents. This creates significant garbage collection overhead and runs the risk of a `RangeError: Maximum call stack size exceeded` for users with large trade histories.
**Action:** Replace chained functional array operations and spread operators with single-pass O(N) `for` loops to compute metrics (like `totalProfit`, `winRate`, `bestTrade`, and `avgConfidence`) simultaneously. This avoids intermediate array allocations and prevents stack overflow errors.
