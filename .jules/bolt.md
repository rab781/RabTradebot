...
## 2026-05-18 - [Optimize DB Stat Metrics & Prevent Stack Overflows]
**Learning:** In `src/services/databaseService.ts`, statistical methods like `getUserTradeStats` used chained functional methods (`.map()`, `.filter()`, `.reduce()`) to process arrays of database records. This caused multiple linear passes plus excess garbage collection from intermediate arrays, and using the spread operator (`Math.max(...profits)`) on large datasets threw a `Maximum call stack size exceeded` RangeError.
**Action:** Always replace chained array operations and `Math.max/min(...array)` with a single, unified `for` loop when calculating aggregate statistics (like counts, sums, min, max) on datasets that can grow unbounded to reduce passes, allocations, and constant-factor overhead while preserving linear O(N) complexity.
