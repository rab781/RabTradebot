...
## 2024-05-04 - Statistical Identity Optimization
**Learning:** In hot loops like `strategyOptimizer.ts`, computing standard deviation using multiple array passes (`.map` followed by two `.reduce` calls) creates unnecessary overhead from both closures and array iterations.
**Action:** Use a single O(N) `for` loop to accumulate standard sums (`sum`) and sums of squares (`sumSq`). Then apply the variance identity `Var(X) = E[X^2] - (E[X])^2` to compute variance and standard deviation in O(1) time after the loop, effectively eliminating redundant iterations and intermediate object creation.
