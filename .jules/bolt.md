...
## 2026-05-03 - [Optimize Hot-Path Array Reduction in Trading Engines]
**Learning:** Inside `BacktestEngine`, calculating the total unrealized PNL for every tick using `openTrades.reduce(...)` created a significant O(N) allocation bottleneck on the hot path due to frequent intermediate closure creation and garbage collection pressure, similar to previous findings in `PaperTradingEngine`.
**Action:** Replace `reduce()` in high-frequency trading loops with single-pass block-scoped `for` loops that accumulate values without closures.
