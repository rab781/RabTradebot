...
## 2024-05-30 - Optimize DataFrame construction
**Learning:** When extracting multiple columns (open, high, low, close) from an array of arrays or objects, chaining `.map()` calls creates multiple O(N) traversals and intermediate arrays, hurting performance.
**Action:** Pre-allocate fixed-size arrays using `new Array(len)` and populate them in a single O(N) `for` loop to avoid closure overhead and redundant iterations.
