...
## 2026-06-03 - [Optimize Extracting Param Values and Computing Stats]
**Learning:** In `StrategyOptimizer.analyzeResults`, calculating statistical metrics over optimization results using multiple `.reduce()` and `.map()` passes added unnecessary O(N) iteration overhead. Also, mapping parameter values inside a loop over parameters led to repeated allocations per parameter evaluated.
**Action:** Consolidate the calculation of sum, sum of squares, and array pre-allocation into a single O(N) `for` loop. Hoist the extraction of metrics out of the parameter loop to prevent redundant allocations and compute mean and variance using a single pass identity `Var(X) = (SumSq - Sum*Sum/N) / N` wrapped in `Math.max(0, variance)`.
