...
## 2026-03-05 - [Replacing slice().map() arrays with inline min/max tracking]
**Learning:** In the technical analysis services, performing `Math.min(...candles.slice(-5).map(c => c.low))` creates two intermediate arrays and hits spread-operator execution limits on larger slices.
**Action:** Replace `slice().map()` min/max tracking with a simple loop over the trailing `N` elements tracking the minimum and maximum inline.
