...
## 2024-05-24 - [O(1) Percentile Extraction Optimization]
**Learning:** In simulation logic (like Monte Carlo testing in `strategyOptimizer`), calculating multiple percentiles iteratively using `.sort()` for each percentile causes redundant O(N log N) overhead.
**Action:** Always sort the distribution array exactly once, and extract all required percentiles simultaneously using pre-calculated O(1) index lookups.
