...
## 2026-06-23 - Optimize percentile extraction in StrategyOptimizer
**Learning:** In the StrategyOptimizer's Monte Carlo robustness test, the `getPercentile` function sorted the entire array of simulation results and calculated an index for each percentile call independently. This resulted in O(N log N) sorting operations being executed multiple times (e.g., 5 times for profit, 5 times for drawdown, 5 times for Sharpe) per set of simulations, which adds significant overhead when the number of simulations is large.
**Action:** Always combine operations when extracting multiple percentiles from the same distribution array. Sort the array exactly once, calculate all the necessary indices in O(1) time based on the single sorted array, and return all percentiles simultaneously in an object. This reduces redundant sorting and improves the execution time of the Monte Carlo loop.
