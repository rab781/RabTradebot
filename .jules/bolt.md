...
## 2025-05-25 - [Optimize Iterative Array Statistics Operations]
**Learning:** In algorithms like `StrategyOptimizer.parameterSensitivity`, computing standard deviation by chaining `.map()`, `Math.max(...array)`, and multiple `.reduce()` iterations incurs significant O(N) allocation and garbage collection overhead. Since the target array size dynamically scales with input, sequential mapping causes CPU bottlenecking and excessive intermediate array closures.
**Action:** Replace functional map/reduce chains with a single O(N) loop to accumulate mathematical statistics. Use the variance identity `Var(X) = E[X^2] - (E[X])^2`, wrapped in `Math.max(0, variance)` to avoid floating-point negative zero NaNs, to calculate standard deviation efficiently without creating intermediate arrays.
