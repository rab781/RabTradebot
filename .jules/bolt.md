...
## 2026-07-31 - Replace multiple O(N) Array .map() with pre-allocated loops in hot logic
**Learning:** Using multiple `.map()` calls on the same array to extract different properties creates multiple O(N) iterations and function closures. In hot paths (like setting up DataFrames or OHLCV arrays), this overhead can accumulate and negatively impact parsing performance.
**Action:** When extracting multiple columns or properties from a list of items, pre-allocate the final arrays (`new Array(len)`) and populate them simultaneously within a single standard `for` loop to reduce time complexity to a single O(N) pass.
