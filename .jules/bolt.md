...
## 2026-07-04 - [Optimize Parameter Correlation and Statistical Variance]
**Learning:** Using multiple `.reduce()` and `.map()` passes for statistical metrics (mean, std dev) and correlations over the same arrays adds unnecessary iteration overhead and closure allocations, significantly slowing down parameter grid searches in `StrategyOptimizer`.
**Action:** Consolidate the calculation of sum, sum of squares, and correlations into a single `O(N)` loop. Mathematically calculate variance as `E[X^2] - (E[X])^2` to avoid secondary iteration passes.
