...
## 2026-06-01 - [Statistical Accumulation in Hot Loops]
**Learning:** Using multiple `.reduce()` and `.map()` passes for statistical metrics (mean, std dev) over the same arrays adds unnecessary iteration overhead, and `Math.max(...array)` introduces severe stack limits for large datasets.
**Action:** Consolidate the calculation of sum, sum of squares, and max value into a single `O(N)` loop. Use `Var(X) = E[X^2] - (E[X])^2` (wrapping in `Math.max(0, variance)` to avoid NaNs) to compute standard deviation without a second pass.
...
## 2026-06-08 - [O(N) Optimization for Parameter Importance Mapping]
**Learning:** In \`StrategyOptimizer.analyzeResults\`, dynamically mapping scores and calculating statistical correlations per parameter iteration created an enormous allocation and garbage collection bottleneck. Specifically, using \`results.map()\` to extract \`paramValues\` and \`scores\` inside a loop, followed by multiple chained \`.reduce()\` calls for calculating mean and standard deviation, forced the engine into an O(P * N * 2) array creation and traversal pattern (where P is params, N is evaluations).
**Action:** When performing statistical analysis on object arrays (like optimization results), never re-extract identical arrays inside a loop. Hoist the creation of common arrays (like \`scores\`) and the calculation of base statistics (like mean and variance via \`sum\` and \`sumSq\`) into a single O(N) pre-pass using pre-allocated arrays (\`new Array(n)\`).
