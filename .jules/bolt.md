...
## 2024-07-23 - Avoid redundant array mappings in parameter iteration loops
**Learning:** In optimization analysis methods (`StrategyOptimizer.analyzeResults`), dynamically calling `.map()` to extract overall scores inside a parameter loop creates O(K*N) redundant memory allocations and traversals, scaling poorly with the number of parameters (K) and results (N).
**Action:** Hoist shared array extractions and statistical calculations outside the parameter loop, and use pre-allocated `new Array(len)` for parameter values to eliminate `.map()` and `.reduce()` overhead.
