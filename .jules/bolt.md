...
## 2026-03-05 - [Optimize Hot Loops by Replacing .reduce()]
**Learning:** In highly iterative scenarios like backtesting engines (e.g., `BacktestEngine` iterating over historical data), using `Array.prototype.reduce` to calculate metrics like `totalUnrealizedPnl` inside the inner loop is surprisingly slow. The V8 engine has to allocate a closure and invoke the callback function for every element on every tick, adding significant overhead.
**Action:** Always replace `.reduce` with a standard block-scoped `for` loop (using an index variable like `j` to avoid collisions) when iterating over collections in hot execution loops to bypass function allocation overhead.
