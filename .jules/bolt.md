...
## 2026-06-05 - [Optimize Iterative Array Mapping for Parameter Analysis]
**Learning:** In `StrategyOptimizer.analyzeResults`, calculating statistical correlation over a parameter grid using `results.map(r => r.score)` inside a `for` loop over parameter names resulted in the identical `scores` array being redundantly mapped, allocated, and garbage collected O(P * N) times (where P is number of parameters).
**Action:** When performing statistical analysis across multiple parameters, always hoist shared array extractions (like mapping overall scores) outside of the parameter iteration loops to prevent redundant O(N) memory allocations per parameter. Additionally, consolidate statistical reductions (sum, mean, std) into the same initial pass to minimize iterations.
