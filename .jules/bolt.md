...
## 2026-06-03 - [Optimize Monte Carlo Percentile Sorting]
**Learning:** In `strategyOptimizer.ts`, the `getPercentile` helper function was repeatedly sorting the entire distribution array for each individual percentile calculation (p5, p25, median, p75, p95) across multiple metrics (profit, drawdown, sharpe). This caused an excessive 15x O(N log N) sorting overhead per simulation batch.
**Action:** When evaluating multiple percentiles from a distribution array in simulation logic, sort the distribution array exactly once and extract all percentiles simultaneously in O(1) time using pre-calculated index lookups.
