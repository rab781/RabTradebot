...
## 2026-07-12 - [Optimize Monte Carlo Percentile Calculation]
**Learning:** In simulation logic (like Monte Carlo testing), using an inline function like `getPercentile` that performs `[...data].sort()` for every percentile (p5, p25, median, p75, p95) results in redundant O(N log N) sorting overhead. Additionally, calling `data.sort()` directly for the median mutates the original array.
**Action:** When evaluating multiple percentiles from a distribution array, sort the distribution array exactly once and extract all percentiles simultaneously in O(1) time using pre-calculated index lookups to avoid redundant sorting and array mutations.
