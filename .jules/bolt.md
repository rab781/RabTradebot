...
## 2026-05-29 - [O(N*M) Allocation Bottlenecks in Optimization Loops]
**Learning:** In \`strategyOptimizer.ts\`, operations that rely on functional array chaining (e.g., \`reduce\` combined with \`map\` or spreading) within optimization loops create severe O(N*M) overhead and excessive garbage collection. Specifically, computing metrics like standard deviation and correlation using chained \`reduce()\` calls on large result sets causes unnecessary multiple passes over the same arrays.
**Action:** Replace functional array method chains (\`reduce\`, \`map\`, \`filter\`) with single O(N) \`for\` loops to accumulate all necessary sums and statistics directly in one pass. This minimizes iteration overhead and avoids repeated closure and intermediate array allocations, significantly improving performance in tight optimization loops.
