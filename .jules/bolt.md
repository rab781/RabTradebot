...
## 2025-05-25 - [Optimize O(N) Iteration Overhead in Statistical Aggregation]
**Learning:** In `StrategyOptimizer.parameterSensitivityAnalysis`, calculating the mean, best performance, and standard deviation via separate chained array methods (`.map()`, `.reduce()`, `.reduce()`) combined with `Math.max(...spread)` creates huge iteration overhead. It traverses the array multiple times, allocates intermediate arrays, relies on `.reduce` closures, and the `Math.max(...spread)` can trigger stack size overflow on large optimization bounds.
**Action:** Replace sequential statistical aggregations and `Math.max(...array)` spreads with a single O(N) `for` loop that accumulates `sum`, `sumSq`, and max values inline. Use `Math.sqrt(Math.max(0, variance))` identities to extract metrics at the end safely.
