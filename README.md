...
**Prerequisites**: Node.js 20.19+, npm 9+, A Telegram Bot Token (get it from [@BotFather](https://t.me/BotFather))
...
| `BINANCE_API_KEY` | `string` | - | Required for live trading and better rate limits. Note: The bot automatically falls back to the public Binance API if private credentials are not provided. |
...
| `WEB_PORT` | `number` | `3000` | Port for the Web Dashboard REST API |
| `WEB_HOST` | `string` | `0.0.0.0` | Host for the Web Dashboard REST API |
| `CORS_ORIGIN` | `string` | `http://localhost:3000, http://127.0.0.1:3000` | Comma-separated list of allowed CORS origins for the API |

## API Reference

The bot provides a REST API and WebSocket connection for real-time updates (e.g. for a web dashboard).
