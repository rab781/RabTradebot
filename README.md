...
The bot provides a REST API on port `3000` (or `WEB_PORT`) for dashboard integrations.

### Authentication
Currently, the REST API runs without authentication, as it serves read-only dashboard data locally. However, ensure that you restrict network access via CORS or firewalls if exposing the API externally.

### Rate Limiting
Requests are limited to **100 requests per minute** per IP address. Exceeding this limit returns an HTTP `429 Too Many Requests` status code.

### Pagination
List endpoints accept a `limit` query parameter (defaulting to 20 or 50) to control the number of returned records.

### Error Handling
Errors return standard HTTP status codes (e.g., `400 Bad Request`, `500 Internal Server Error`) along with a JSON body:
```json
{
...
- **`GET /api/news?limit=20`**
  Returns latest cryptocurrency news and sentiment analysis.

- **`GET /api/portfolio`**
  Returns current portfolio balance, simulated holdings, and summary statistics.

- **`GET /api/stats`**
  Returns overall bot operational statistics and performance metrics.

- **`GET /api/health`**
  Returns bot uptime and status (HTTP 200).

- **`GET /health`**
  Returns detailed health monitoring payload for external systems.
