...
describe('F6: RateLimiter', () => {
    ...
    it('syncs REST tokens from Binance used weight header', () => {
        ...
        const snapshot = limiter.getSnapshot();
        expect(snapshot.restTokens).toBeLessThan(11); // Allow fractional timing differences
    });
    ...
    it('syncs ORDER tokens from order count header', () => {
        ...
        const snapshot = limiter.getSnapshot();
        expect(snapshot.orderTokens).toBeLessThan(2); // Allow fractional timing differences
    });
