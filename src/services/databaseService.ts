...
function applyPostgresPoolParams(rawUrl: string): string {
  ...
  const poolTimeout =
    Number.isFinite(poolTimeoutRaw) && poolTimeoutRaw > 0 ? Math.floor(poolTimeoutRaw) : 5;
...
export class DatabaseService {
  ...
  async saveStrategyMetricsFromBacktest(backtest: {
    ...
    const avgProfit =
      backtest.profitableTrades > 0 ? backtest.totalProfit / backtest.profitableTrades : 0;
    const avgLoss = backtest.lossTrades > 0 ? (backtest.totalLoss || 0) / backtest.lossTrades : 0;
   ...
   */
  async getUnverifiedPredictions(minAge: number = 3600000) {
    // 1 hour in ms
