...
export class TechnicalAnalyzer {
    ...
    async analyzeSymbol(symbol: string): Promise<string> {
        ...
        try {
            ...
            // ⚡ Bolt Optimization: Replace chained array methods and intermediate allocations
            // with a single pre-allocated loop for parsing OHLVC data.
            const len = candlesData.length;
            const candles: Candle[] = new Array(len);
            const closePrices = new Array<number>(len);
            ...
            // ⚡ Bolt Optimization: Replace slice().reduce() with direct loop
            let volSum = 0;
            const volCount = Math.min(20, len);
            for (let i = len - volCount; i < len; i++) {
                volSum += volumes[i];
            }
            const volumeAvg = volCount > 0 ? volSum / volCount : 0;
            ...
            if (signal.includes('BUY')) {
                // ⚡ Bolt Optimization: Avoid slice, map, and Math.min spread overhead
                ...
                // ⚡ Bolt Optimization: Avoid slice, map, and Math.max spread overhead
                let stopLoss = -Infinity;
                const lookback = Math.min(5, len);
                for (let i = len - lookback; i < len; i++) {
