...
export class BinanceOrderService {
  ...
  constructor() {
    ...
    this.rateLimiterTimeoutMs = parseInt(
      process.env.BINANCE_RATE_LIMITER_TIMEOUT_MS || '30000',
      10
    );
  }
  ...
  async getAccountBalance(): Promise<BinanceBalance[]> {
    const account = await this.privateRequest<{ balances: BinanceBalance[] }>(
      'GET',
      '/api/v3/account',
      undefined,
      10
  ...
  async getSymbolInfo(symbol: string): Promise<SymbolTradeRules> {
    ...
    }>('/api/v3/exchangeInfo', { symbol: symbol.toUpperCase() }, 10);
  ...
  private async privateRequest<T>(
    ...
    if (!this.isConfigured()) {
      throw new Error(
        'Binance API keys are not configured. Set BINANCE_API_KEY and BINANCE_API_SECRET.'
      );
    }
