...
export class BayesianOptimizer {
  ...
  async optimize(): Promise<StrategyOptimizationResult[]> {
    logger.info(
      `Starting Bayesian Optimization (TPE) with max ${this.config.maxEvals} evaluations`
    );
    ...
    logger.info(
      `Phase 2: Guided exploration with TPE for ${this.config.maxEvals - initialRandomEvals} evaluations...`
    );
    ...
    for (let i = initialRandomEvals; i < this.config.maxEvals; i++) {
      ...
      if ((i + 1) % 10 === 0 || i === this.config.maxEvals - 1) {
        logger.info(
          `TPE phase: ${i + 1}/${this.config.maxEvals} (Best: ${bestScoreSoFar.toFixed(4)})`
        );
      }
    }
    ...
    for (let i = 0; i < Math.min(5, results.length); i++) {
      logger.info(
        `${i + 1}. Score: ${results[i].score.toFixed(4)}, Params: ${JSON.stringify(results[i].params)}`
      );
    }
  ...
  static getEfficiencyReport(
    ...
    const bayesianImprovement =
      ((bayesianBest.score - gridBest.score) / Math.abs(gridBest.score)) * 100;
