...
export class NewsAnalyzer {
  ...
  async analyzeComprehensiveNews(
    symbol: string,
    currentPrice?: number
  ): Promise<NewsAnalysisResult> {
    const coinName = this.getBaseCurrency(symbol);
    logger.info(`[SCRAPE] [START] symbol=${symbol} coin=${coinName}`);
    ...
    const cryptoPanicCount =
      cryptoPanicResult.status === 'fulfilled' ? cryptoPanicResult.value.length : 0;
    ...
    if (rssResult.status === 'rejected') {
      logger.warn(
        `[SCRAPE] [RSS][ERROR] symbol=${symbol} message=${rssResult.reason?.message || rssResult.reason}`
      );
    }
    if (redditResult.status === 'rejected') {
      logger.warn(
        `[SCRAPE] [REDDIT][ERROR] symbol=${symbol} message=${redditResult.reason?.message || redditResult.reason}`
      );
    }
    if (cryptoPanicResult.status === 'rejected') {
      logger.warn(
        `[SCRAPE] [CRYPTOPANIC][ERROR] symbol=${symbol} message=${cryptoPanicResult.reason?.message || cryptoPanicResult.reason}`
      );
    }
    ...
    const combinedScore =
      articles.length > 0 && reddit.length > 0
    ...
    if (this.chutesService?.isConfigured() && (articles.length > 0 || reddit.length > 0)) {
      try {
        logger.info(
          `[AI] [START] symbol=${symbol} articles=${articles.length} reddit=${reddit.length}`
        );
        ...
        logger.info(
          `[AI] [DONE] symbol=${symbol} sentiment=${aiAnalysis.overallSentiment} direction=${aiAnalysis.marketMovement.direction} confidence=${aiAnalysis.marketMovement.confidence.toFixed(1)}%`
        );
      } catch (err: any) {
        logger.warn(`[AI] [ERROR] symbol=${symbol} message=${err.message}`);
      }
    } else {
      logger.info(
        `[AI] [SKIP] symbol=${symbol} configured=${this.chutesService?.isConfigured() ? 'yes' : 'no'} articles=${articles.length} reddit=${reddit.length}`
      );
    }
  ...
  formatBasicAnalysis(result: NewsAnalysisResult): string {
    ...
    if (result.aiAnalysis) {
      ...
      out += `📋 Key Factors:\n`;
      ai.keyFactors.slice(0, 3).forEach((f, i) => {
        out += `${i + 1}. ${f}\n`;
      });
      out += `\n⏰ Impact:\n`;
  ...
  private async fetchRSSFeeds(coinName: string): Promise<NewsItem[]> {
    ...
    return articles.sort((a, b) => b.publishedAt.getTime() - a.publishedAt.getTime()).slice(0, 25);
  }
  ...
  private async fetchSingleRSS(
    ...
    $('item').each((_, el) => {
      ...
      const link =
        $(el).find('link').first().text().trim() || $(el).find('link').attr('href') || '';
