...
export class BinanceService {
  ...
  constructor() {
    ...
    if (proxyUrl) {
      ...
      if (this.proxyConfig) {
        ...
        logger.info(
          `🌐 [BinanceService] Using proxy ${this.proxyConfig.protocol}//${this.proxyConfig.host}:${this.proxyConfig.port} (${proxyAuth})`
        );
      } else {
    ...
    if (tlsInsecure) {
      ...
      logger.warn(
        '⚠️  [BinanceService] BINANCE_TLS_INSECURE=true, TLS certificate verification is disabled'
      );
    } else if (caCertPath) {
      try {
        ...
        logger.error(
          `❌ [BinanceService] Failed to load BINANCE_CA_CERT_PATH (${caCertPath}): ${(error as Error).message}`
        );
      }
    }
    ...
    if (!this.apiKey || !this.apiSecret) {
      logger.warn(
        '⚠️  [BinanceService] BINANCE_API_KEY / BINANCE_API_SECRET not set in environment'
      );
    }
  ...
  formatHealthReport(status: BinanceHealthStatus): string {
    ...
    // ── Public API block ──────────────────
    const pubStatusLine = publicApi.reachable ? '✅ Online' : '❌ Offline';

    const latencyEmoji = !publicApi.reachable
      ? '⚫'
        ...
        const rows = privateApi.balances
          .map(
            (b) =>
              ...
              (parseFloat(b.locked) > 0 ? `  locked: ${parseFloat(b.locked).toFixed(6)}` : '')
          )
          .join('\n');
        balancesBlock = `\n💰 Non-zero balances:\n${rows}`;
      } else {
    ...
    if (rateLimits && rateLimits.length > 0) {
      const rows = rateLimits
        .map(
          (r) => `  • ${r.rateLimitType}: ${r.limit} / ${r.intervalNum}${r.interval.toLowerCase()}`
        )
        .join('\n');
      rateLimitsBlock = `\n📊 Rate limits:\n${rows}`;
    }
  ...
  private parsePrivateError(error: any): string {
    ...
    if (binanceCode === -1021) return 'Timestamp out of sync – your system clock may be drifting';
