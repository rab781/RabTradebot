...
- Best for risk minimization: ${result.frontier.reduce((min, p) => p.objectives.drawdownPct < min.objectives.drawdownPct ? p : min).objectives.drawdownPct.toFixed(1)}% maximum drawdown
    ...
    monteCarloTest(trades: Array<{profit: number; maxDrawdown: number}>, numSimulations: number = 1000): MonteCarloResult {
        ...
        // ⚡ Bolt Optimization: Sort exactly once per distribution to avoid O(N log N) overhead on every percentile fetch
        const getSortedAndPercentiles = (data: number[]) => {
            const sorted = [...data].sort((a, b) => a - b);
            const len = sorted.length;
            return {
                p5: sorted[Math.max(0, Math.ceil(0.05 * len) - 1)] || 0,
                p25: sorted[Math.max(0, Math.ceil(0.25 * len) - 1)] || 0,
                median: sorted[Math.floor(len / 2)] || 0,
                p75: sorted[Math.max(0, Math.ceil(0.75 * len) - 1)] || 0,
                p95: sorted[Math.max(0, Math.ceil(0.95 * len) - 1)] || 0
            };
        };

        const result: MonteCarloResult = {
            simulations: numSimulations,
            profitDistribution: getSortedAndPercentiles(profitResults),
            drawdownDistribution: getSortedAndPercentiles(drawdownResults),
            sharpeDistribution: getSortedAndPercentiles(sharpeResults),
            summary: ''
        };
