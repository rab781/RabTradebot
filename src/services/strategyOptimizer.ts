...
- Best for risk minimization: ${result.frontier.reduce((min, p) => p.objectives.drawdownPct < min.objectives.drawdownPct ? p : min).objectives.drawdownPct.toFixed(1)}% maximum drawdown
    ...
    monteCarloTest(trades: Array<{profit: number; maxDrawdown: number}>, numSimulations: number = 1000): MonteCarloResult {
        ...
        // ⚡ Bolt Optimization: Sort distribution arrays exactly once and extract all percentiles
        // simultaneously in O(1) time using pre-calculated index lookups to avoid
        // redundant O(N log N) sorting overhead.
        ...
        const getPercentileSorted = (sorted: number[], p: number) => {
        ...
        const result: MonteCarloResult = {
            simulations: numSimulations,
            profitDistribution: {
                p5: getPercentileSorted(sortedProfits, 5),
                p25: getPercentileSorted(sortedProfits, 25),
                median: getPercentileSorted(sortedProfits, 50),
                p75: getPercentileSorted(sortedProfits, 75),
                p95: getPercentileSorted(sortedProfits, 95)
            },
            drawdownDistribution: {
                p5: getPercentileSorted(sortedDrawdowns, 5),
                p25: getPercentileSorted(sortedDrawdowns, 25),
                median: getPercentileSorted(sortedDrawdowns, 50),
                p75: getPercentileSorted(sortedDrawdowns, 75),
                p95: getPercentileSorted(sortedDrawdowns, 95)
            },
            sharpeDistribution: {
                p5: getPercentileSorted(sortedSharpes, 5),
                p25: getPercentileSorted(sortedSharpes, 25),
                median: getPercentileSorted(sortedSharpes, 50),
                p75: getPercentileSorted(sortedSharpes, 75),
                p95: getPercentileSorted(sortedSharpes, 95)
            },
            summary: ''
        };
