...
${Object.entries(parameterImportance)
    ...
    private static calculateCorrelation(x: number[], y: number[]): number {
        ...
        const n = x.length;
        // ⚡ Bolt Optimization: Replace 5 chained .reduce() calls with a single O(N) loop
        let sumX = 0;
        let sumY = 0;
        let sumXY = 0;
        let sumX2 = 0;
...
- Best for risk minimization: ${result.frontier.reduce((min, p) => p.objectives.drawdownPct < min.objectives.drawdownPct ? p : min).objectives.drawdownPct.toFixed(1)}% maximum drawdown
    ...
    parameterSensitivity(results: StrategyOptimizationResult[], bestParams: Record<string, any>): SensitivityResult[] {
        ...
        for (const [paramName, optimalValue] of Object.entries(bestParams)) {
            ...
            // ⚡ Bolt Optimization: Calculate variance mathematically with a single pass
            // using Var(X) = E[X^2] - (E[X])^2 instead of .map(), .reduce(), and multiple iterations
            let sumScore = 0;
            let sumSqScore = 0;
            let bestPerf = -Infinity;
            const n = neighborhood.length;
            const performances: number[] = new Array(n);
            ...
            const mean = sumScore / n;
            // Var(X) = E[X^2] - (E[X])^2
            const variance = (sumSqScore / n) - (mean * mean);
            // Protect against floating point precision errors causing negative variance
