...
export class StrategyOptimizer {
  ...
  async optimize(): Promise<StrategyOptimizationResult[]> {
    ...
    for (let i = 0; i < totalCombinations; i++) {
      ...
      try {
        ...
        const backtestConfig = {
          ...
          stakeAmount:
            typeof modifiedStrategy.stakeAmount === 'number' ? modifiedStrategy.stakeAmount : 100,
        ...
        if ((i + 1) % 10 === 0 || i === totalCombinations - 1) {
          logger.info(
            `Progress: ${i + 1}/${totalCombinations} (${(((i + 1) / totalCombinations) * 100).toFixed(1)}%)`
          );
    ...
    for (let i = 0; i < Math.min(10, results.length); i++) {
      ...
      logger.info(
        `${i + 1}. Score: ${result.score.toFixed(4)}, Params: ${JSON.stringify(result.params)}`
      );
      logger.info(
        `   Total Profit: ${result.backtestResult.totalProfitPct.toFixed(2)}%, Win Rate: ${result.backtestResult.winRate.toFixed(1)}%`
      );
    }
  ...
  static analyzeResults(results: StrategyOptimizationResult[]): {
    ...
    const scoreStd = Math.sqrt(
      results.reduce((sum, r) => sum + Math.pow(r.score - avgScore, 2), 0) / results.length
    );
...
${Object.entries(parameterImportance)
  ...
  private static calculateCorrelation(x: number[], y: number[]): number {
    ...
    const n = x.length;
    // ⚡ Bolt Optimization: Replace O(5N) chained reduce operations with a single O(N) loop
  ...
  async walkForwardOptimize(
    ...
    logger.info(
      `Starting Walk-Forward Optimization with ${numWindows} windows, ${(inSampleRatio * 100).toFixed(0)}% in-sample`
    );
    ...
    for (let w = 0; w < numWindows; w++) {
      ...
      const backtestEngine = new BacktestEngine(modifiedStrategy, {
        ...
        stakeAmount:
          typeof (modifiedStrategy as any).stakeAmount === 'number'
            ? (modifiedStrategy as any).stakeAmount
            : 100,
        startingBalance: 1000,
        feeOpen: 0.001,
        feeClose: 0.001,
    ...
    const avgStabilityRatio =
      windows.reduce((sum, w) => sum + w.stabilityRatio, 0) / windows.length;
    const summary = this.formatWFOSummary({
      windows,
  ...
  private formatWFOSummary(result: WFOResult): string {
    const windowSummaries = result.windows
      .map(
        (w) =>
...
Higher ratio (closer to 1.0) indicates less overfitting.
  ...
  static deflatedSharpeRatio(
    ...
    const denomPart =
      1 - returnSkewness * observedSR + ((returnKurtosis - 1) / 4) * observedSR * observedSR;
  ...
  private formatParetoSummary(result: ParetoResult): string {
    const frontierDetails = result.frontier
      .map(
        (p) =>
...
- Best for risk minimization: ${result.frontier.reduce((min, p) => (p.objectives.drawdownPct < min.objectives.drawdownPct ? p : min)).objectives.drawdownPct.toFixed(1)}% maximum drawdown
  ...
  parameterSensitivity(
    ...
    for (const [paramName, optimalValue] of Object.entries(bestParams)) {
      ...
      const mean = sumPerf / n;
      const variance = Math.max(0, sumPerfSq / n - mean * mean);
   ...
   */
  monteCarloTest(
    trades: Array<{ profit: number; maxDrawdown: number }>,
    ...
    // ⚡ Bolt Optimization: Sort exactly once per distribution to avoid redundant O(N log N) overhead
