...
export class FeatureEngineeringService {
  ...
  private calculateAdvancedStats(values: number[], startIndex: number = 0, length?: number) {
    ...
    if (stdDev !== 0) {
      ...
      if (n > 3) {
        kurtosis =
          ((n * (n + 1)) / ((n - 1) * (n - 2) * (n - 3))) * (sumQuartDiff / Math.pow(stdDev, 4)) -
  ...
  private calculateCorrelation(
    ...
    const denominator = Math.sqrt(
      Math.max(0, (n * sumX2 - sumX * sumX) * (n * sumY2 - sumY * sumY))
    );
  ...
  addMultiTimeframeFeatures(
    ...
    const bb15m =
      closes15m.length >= 20
        ? BollingerBands.calculate({ period: 20, values: closes15m, stdDev: 2 })
        : [];
    const volMA15m =
      volumes15m.length >= 20 ? SMA.calculate({ period: 20, values: volumes15m }) : [];
    ...
    const atr4h =
      highs4h.length >= 15
        ? ATR.calculate({ high: highs4h, low: lows4h, close: closes4h, period: 14 })
        : [];
    ...
    return baseFeatures.map((base) => {
      ...
      const bb_pctB_15m =
        bbObj && bbObj.upper !== bbObj.lower
          ? (c15 - bbObj.lower) / (bbObj.upper - bbObj.lower)
          : 0.5;
