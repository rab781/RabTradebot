...
export interface SignalStrategy {
  ...
  populateEntryTrend?: (
    data: any[]
  ) => { action?: 'BUY' | 'SELL' | 'HOLD'; confidence?: number; reason?: string } | null;
}
...
export class RiskMonitorLoop {
  ...
  startWebSocket(symbols: string[]): void {
    if (!this.wsService) {
      withLogContext({ service: 'riskMonitorLoop' }).warn(
        'No wsService set, cannot start WebSocket mode'
      );
      return;
    }
    ...
    for (const symbol of symbols) {
      ...
      this.wsService.subscribeTickerStream(symUpper, async (tick) => {
        ...
        try {
          ...
          const symbolTrades = trades.filter((t: any) => t.symbol.toUpperCase() === symUpper);
  ...
  private async evaluateExitTriggers(trade: any, currentPrice: number): Promise<void> {
    ...
    if (stopTriggered) {
      ...
      await this.notify(
        `🛑 Stop loss triggered for ${trade.symbol} @ ${currentPrice.toFixed(4)}`,
        trade.userId
      );
    ...
    if (takeProfitTriggered) {
      ...
      await this.notify(
        `🎯 Take profit reached for ${trade.symbol} @ ${currentPrice.toFixed(4)}`,
        trade.userId
      );
      this.trailState.delete(trade.id);
    }
