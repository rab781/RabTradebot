...
export class SimpleComprehensiveAnalyzer {
  ...
  async analyzeComprehensive(symbol: string): Promise<SimpleAnalysisResult> {
    ...
    try {
      ...
      const [marketDataRaw, srLevels, volumeAnalysis, marketData1h, timeframes] = await Promise.all(
        [
    ...
    try {
      ...
      return {
        ...
        technical: {
          ...
          bollinger: {
            position:
              (realAnalysis.rsi || 50) > 70
                ? 'above'
                : (realAnalysis.rsi || 50) < 30
                  ? 'below'
                  : 'middle',
            squeeze: false,
            breakout: false,
          ...
          supportResistance: {
            ...
            distanceToSupport:
              realAnalysis.currentPrice && realAnalysis.support
                ? ((realAnalysis.currentPrice - realAnalysis.support) / realAnalysis.currentPrice) *
                  100
                : 0,
            distanceToResistance:
              realAnalysis.currentPrice && realAnalysis.resistance
                ? ((realAnalysis.resistance - realAnalysis.currentPrice) /
                    realAnalysis.currentPrice) *
                  100
                : 0,
          },
          volume: {
          ...
          movingAverages: {
            ...
            alignment:
              (realAnalysis.currentPrice || 0) > (realAnalysis.ema10 || 0) ? 'bullish' : 'bearish',
          },
        },
        timeframes: {
          '1h': {
            ...
            signal:
              realAnalysis.timeframes['1h'] === 'bullish'
                ? 'buy'
                : realAnalysis.timeframes['1h'] === 'bearish'
                  ? 'sell'
                  : 'hold',
            strength: 0.7,
          },
          '4h': {
            ...
            signal:
              realAnalysis.timeframes['4h'] === 'bullish'
                ? 'buy'
                : realAnalysis.timeframes['4h'] === 'bearish'
                  ? 'sell'
                  : 'hold',
            strength: 0.7,
          },
          '1d': {
            ...
            signal:
              realAnalysis.timeframes['1d'] === 'bullish'
                ? 'buy'
                : realAnalysis.timeframes['1d'] === 'bearish'
                  ? 'sell'
                  : 'hold',
            strength: 0.7,
          },
  ...
  private async getMultiTimeframeAnalysis(
    symbol: string
  ): Promise<{ '1h': string; '4h': string; '1d': string }> {
  ...
  private generateRecommendation(
    ...
    const bullishTF = Object.values(timeframes).filter(
      (tf: any) => tf.includes('bullish') || tf.includes('oversold')
    ).length;
    const bearishTF = Object.values(timeframes).filter(
      (tf: any) => tf.includes('bearish') || tf.includes('overbought')
    ).length;
    ...
    if (timeframes['4h'] === 'bullish' && timeframes['1d'] === 'bullish')
      directionalBias = 'BULLISH';
    else if (timeframes['4h'] === 'bearish' && timeframes['1d'] === 'bearish')
