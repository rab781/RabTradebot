...
export class BacktestEngine {
  ...
  private closeTrade(trade: Trade, candle: OHLCVCandle, exitReason: string): void {
    ...
    console.log(
      `Closed ${trade.side} trade for ${trade.pair} at ${exitPrice}, profit: ${trade.profit?.toFixed(2)} (${trade.profitPct?.toFixed(2)}%)`
    );
  }
  ...
  private calculateResults(
    ...
    const bestTrade = trades.reduce(
      (best, current) => ((current.profit || 0) > (best?.profit || -Infinity) ? current : best),
      null as Trade | null
    );
    const worstTrade = trades.reduce(
      (worst, current) => ((current.profit || 0) < (worst?.profit || Infinity) ? current : worst),
      null as Trade | null
    );
    ...
    const avgPositiveReturn =
      positiveReturns.length > 0
        ? positiveReturns.reduce((sum, p) => sum + p, 0) / positiveReturns.length
        : 0;
    const avgNegativeReturn =
      negativeReturns.length > 0
        ? negativeReturns.reduce((sum, p) => sum + p, 0) / negativeReturns.length
        : 0;

    const profitFactor =
      avgNegativeReturn !== 0 ? Math.abs(avgPositiveReturn / avgNegativeReturn) : 0;
