    ...
    try {
          ...
          messages: [
            {
              role: 'system',
              content:
                'You are a news researcher tasked with analyzing recent news and trends over the past week. Please write a comprehensive report of the current state of the world that is relevant for trading and macroeconomics. Always provide accurate, structured data in JSON format.',
            },
            {
              role: 'user',
              content: prompt,
      ...
      if (!response.data?.choices?.[0]?.message?.content) {
        logger.error(
          ...
          'Invalid Chutes API response structure'
        );
          ...
          messages: [
            {
              role: 'system',
              content:
                'You are a professional cryptocurrency analyst with 10+ years of trading experience. You provide accurate, conservative predictions based on data analysis, not speculation. Always return valid JSON.',
            },
            {
              role: 'user',
...
Latest: ${news[0].title}`;
  ...
  async analyzeRealNews(
    ...
    const redditContext = redditPosts
      .slice(0, 10)
      .map((p, i) => `[Reddit ${i + 1}] r/${p.subreddit} (↑${p.score}): ${p.title}`)
      .join('\n');
...
Return JSON:
{
  ...
  "sentiment_breakdown": {
    ...
    try {
          ...
          messages: [
            {
              role: 'system',
              content:
                'You are a professional cryptocurrency analyst. Analyze only the provided real scraped news data. Do NOT invent or hallucinate news. Base all predictions strictly on the given articles and Reddit posts. Return valid JSON.',
            },
      ...
      this.setCachedData(cacheKey, analysis);
      logger.info(
        `✅ Real-news AI analysis completed for ${symbol} (${articles.length} articles, ${redditPosts.length} reddit posts)`
      );
      return analysis;
    } catch (error: any) {
