...
export class ConfigManager {
  ...
  private validateConfig(config: any): void {
    const requiredFields = ['trading', 'exchange', 'strategy'];
  ...
  public validateTradingSetup(): { isValid: boolean; errors: string[] } {
    ...
    if (this.config.trading.dry_run_wallet < minBalance) {
      errors.push(
        `dry_run_wallet (${this.config.trading.dry_run_wallet}) is too small for max_open_trades * stake_amount (${minBalance})`
      );
    }
