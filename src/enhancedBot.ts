...
async function handleInlineRun(
  ...
  switch (action) {
    ...
    case 'openclaw': {
      ...
      // ⚡ Bolt Optimization: Pre-allocated loop
      const ocCandles: OHLCVCandle[] = new Array(candles.length);
      ...
      // ⚡ Bolt Optimization: Replaced O(N) multiple .map() closures with pre-allocated loop
      const df: any = {
        open: new Array(ocCandles.length),
        high: new Array(ocCandles.length),
        low: new Array(ocCandles.length),
        close: new Array(ocCandles.length),
        volume: new Array(ocCandles.length),
        date: new Array(ocCandles.length),
      };
      for (let i = 0; i < ocCandles.length; i++) {
        const c = ocCandles[i];
        df.open[i] = c.open;
        df.high[i] = c.high;
        df.low[i] = c.low;
    ...
    case 'mlpredict': {
      ...
      // ⚡ Bolt Optimization: Pre-allocated loop
      const mlOhlcv: OHLCVCandle[] = new Array(mlCandles.length);
      for (let i = 0; i < mlCandles.length; i++) {
        const c = mlCandles[i];
        mlOhlcv[i] = {
          timestamp: c[0],
          open: parseFloat(c[1]),
...
bot.command('openclaw', async (ctx) => {
  ...
  try {
    ...
    // ⚡ Bolt Optimization: Pre-allocated array loop for OHLCV extraction
    const ohlcvCandles: OHLCVCandle[] = new Array(candles.length);
    for (let i = 0; i < candles.length; i++) {
      const c = candles[i];
      ohlcvCandles[i] = {
        timestamp: c[0],
    ...
    // ⚡ Bolt Optimization: Use pre-allocated arrays to avoid O(N) multiple .map() closures
    const dataframe: any = {
      open: new Array(ohlcvCandles.length),
      high: new Array(ohlcvCandles.length),
      low: new Array(ohlcvCandles.length),
      close: new Array(ohlcvCandles.length),
      volume: new Array(ohlcvCandles.length),
      date: new Array(ohlcvCandles.length),
    };
    for (let i = 0; i < ohlcvCandles.length; i++) {
