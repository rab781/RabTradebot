...
async function handleInlineRun(ctx: any, action: string, symbol: string, chatId: number, telegramId: number, dbUserId: number) {
  switch (action) {
    ...
    case 'openclaw': {
      ...
      // ⚡ Bolt Optimization: Pre-allocate arrays and merge 7 O(N) loops into 1
      ...
      for (let i = 0; i < len; i++) {
        ...
        ocCandles[i] = { timestamp, open, high, low, close, volume, date };
        df.open[i] = open;
        df.high[i] = high;
        df.low[i] = low;
        df.close[i] = close;
...
bot.command('openclaw', async (ctx) => {
  ...
  try {
    ...
    for (let i = 0; i < len; i++) {
      ...
      ohlcvCandles[i] = { timestamp, open, high, low, close, volume, date };
      dataframe.open[i] = open;
      dataframe.high[i] = high;
      dataframe.low[i] = low;
      dataframe.close[i] = close;
      dataframe.volume[i] = volume;
      dataframe.date[i] = date;
    }
