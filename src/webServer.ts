...
const app = express();
app.disable('x-powered-by'); // 🛡️ Sentinel: Disable x-powered-by header
...
app.use((req: Request, res: Response, next: NextFunction) => {
    ...
    res.setHeader('Content-Security-Policy', "default-src 'self'; connect-src 'self' ws: wss:; img-src 'self' data:; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline';");
    next();
});
