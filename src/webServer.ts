...
const app = express();
app.disable('x-powered-by'); // 🛡️ Sentinel: Prevent Information Disclosure by hiding Express version
...
app.use((req: Request, res: Response, next: NextFunction) => {
    ...
    res.setHeader('Content-Security-Policy', "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self' ws: wss:; font-src 'self'; frame-src 'none'; object-src 'none'");
    next();
});
