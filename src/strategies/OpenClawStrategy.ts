...
export class OpenClawStrategy implements IStrategy {
  ...
  populateIndicators(dataframe: DataFrame, metadata: StrategyMetadata): DataFrame {
    ...
    dataframe.macd_signal = new Array(macdPad)
      .fill(0)
      .concat(macdResult.map((m) => m?.signal || 0));
    dataframe.macd_histogram = new Array(macdPad)
      .fill(0)
    ...
    dataframe.bb_upper = new Array(bbPad)
      .fill(closes[0])
      .concat(bbResult.map((b) => b?.upper || 0));
    dataframe.bb_middle = new Array(bbPad)
      .fill(closes[0])
      .concat(bbResult.map((b) => b?.middle || 0));
    dataframe.bb_lower = new Array(bbPad)
      .fill(closes[0])
      .concat(bbResult.map((b) => b?.lower || 0));
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (i >= period) {
        ...
        const variance =
          (rollingReturnSumSq - (rollingReturnSum * rollingReturnSum) / period) / period;
  ...
  private calculateVolumeScore(dataframe: DataFrame, index: number): number {
    ...
    const priceChange =
      ((dataframe.close[index] - dataframe.close[index - 1]) / dataframe.close[index - 1]) * 100;
  ...
  populateExitTrend(dataframe: DataFrame, metadata: StrategyMetadata): DataFrame {
    ...
    for (let i = this.startupCandleCount; i < dataframe.close.length; i++) {
      ...
      if (rsi > 75 || macdHist < 0 || regime === 'trending_bear') {
        (dataframe.exit_long as number[])[i] = 1;
        (dataframe.exit_tag as string[])[i] = 'exit_long';
      }

      // Exit SHORT conditions
      if (rsi < 25 || macdHist > 0 || regime === 'trending_bull') {
