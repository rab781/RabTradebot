...
export class SampleStrategy implements IStrategy {
    ...
    populateEntryTrend(dataframe: DataFrame, metadata: StrategyMetadata): DataFrame {
        ...
        for (let i = 1; i < length; i++) {
            ...
            const rsiCrossedAbove = (dataframe.rsi as number[])[i] > this.buyRsi && 
        ...
        if (this.canShort) {
            for (let i = 1; i < length; i++) {
                const rsiCrossedBelow = (dataframe.rsi as number[])[i] < this.shortRsi && 
    ...
    populateExitTrend(dataframe: DataFrame, metadata: StrategyMetadata): DataFrame {
        ...
        for (let i = 1; i < length; i++) {
            ...
            const rsiCrossedAbove = (dataframe.rsi as number[])[i] > this.sellRsi && 
        ...
        if (this.canShort) {
            for (let i = 1; i < length; i++) {
                const rsiCrossedBelow = (dataframe.rsi as number[])[i] < this.exitShortRsi && 
    ...
    customStoploss(trade: Trade, currentTime: Date, currentRate: number, currentProfit: number): number | null {
        ...
        if (this.trailingStop && currentProfit > 0.01) { // If profit > 1%
    ...
    confirmTradeExit(pair: string, trade: Trade, orderType: string, amount: number, rate: number, time: Date): boolean {
