...
export class SimpleGRUModel {
  ...
  async trainWithSplit(
    ...
    for (let epoch = 0; epoch < epochs; epoch++) {
      ...
      const valAcc =
        (history.history['val_acc'][0] as number) ??
        (history.history['val_accuracy']?.[0] as number);
      const trainLoss = history.history['loss'][0] as number;
      const trainAcc =
        (history.history['acc'][0] as number) ?? (history.history['accuracy']?.[0] as number);
    ...
    const valActuals = valTargets.map((t, i) =>
      this.targetToClass(t) === this.targetToClass(valPreds[i] ?? 0) ? 1 : 0
    );
    this.plattScaler.fit(
      valPreds.map((p) => p),
      valActuals
    );
  ...
  async predict(features: FeatureSet[]): Promise<SimplePrediction> {
    ...
    const direction =
      classIdx === CLASS_UP ? calibratedConf : classIdx === CLASS_DOWN ? -calibratedConf : 0;
  ...
  async walkForwardValidate(
    ...
    for (let start = 0; start + windowSize + stepSize <= features.length; start += stepSize) {
      ...
      logger.info(
        `WFV Window ${window}: accuracy=${(accuracy * 100).toFixed(1)}% loss=${loss.toFixed(4)}`
      );
      window++;
    }
  ...
  summarizeWFV(results: WFVResult[]): WFVSummary {
    ...
    // and spread operators (Math.max(...)) with a single O(N) pass.
    // This eliminates intermediate array allocations and prevents potential
    // 'Maximum call stack size exceeded' errors on large WFV result sets.
    let sum = 0;
    let sumSq = 0;
    let best = -Infinity;
    let worst = Infinity;
  ...
  async calibrationTest(
    ...
    for (let i = this.sequenceLength; i < features.length; i++) {
      ...
      preds.push({
        ...
        correct: CLASS_NAMES.indexOf(pred.predictedClass) === actualClass,
      });
    }
  ...
  private async computeConfusionMatrix(
    features: FeatureSet[],
    targets: number[]
  ): Promise<number[][]> {
